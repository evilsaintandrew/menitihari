...
function GuestCard({ invitationId, groups, guest, canEdit }: { readonly invitationId: string; readonly groups: GuestManagementData["groups"]; readonly guest: GuestManagementItem; readonly canEdit: boolean }) {
  const [editing, setEditing] = useState(false);
  const rsvpSummary = guest.assignedEvents.length === 0
    ? "Belum ada acara"
    : guest.assignedEvents.map((event) => `${event.name}: ${event.rsvpStatus === "ATTENDING" ? `Hadir${event.attendanceCount ? ` ${event.attendanceCount}` : ""}` : event.rsvpStatus === "NOT_ATTENDING" ? "Tidak hadir" : "Belum"}`).join(" · ");
  ...
  return (
    <Card className="guests-card">
      ...
      {editing && <GuestForm canEdit={canEdit} groups={groups} guest={guest} invitationId={invitationId} onSaved={() => setEditing(false)} />}
...
export function GuestsManager({ data }: { readonly data: GuestManagementData }) {
  ...
  return (
    <main className="guests-page">
      ...
      {adding && <Card className="guests-add-card"><CardHeader><p className="ui-overline">WF-11</p><h2>Tambah tamu</h2><CardDescription>Simpan satu penerima atau party dengan cepat. Penugasan acara dikelola pada alur acara tamu.</CardDescription></CardHeader><GuestForm canEdit={data.canEdit} groups={data.groups} guest={null} invitationId={data.invitationId} onSaved={() => setAdding(false)} /></Card>}
      {data.guests.length === 0 ? (
        ...
        <section aria-label="Daftar tamu" className="guests-list">{guests.map((guest) => <GuestCard canEdit={data.canEdit} groups={data.groups} guest={guest} invitationId={data.invitationId} key={guest.id} />)}</section>
      )}
    </main>
  );
}
